...
export function NotebooksPage() {
  ...
  const query = useQuery({ queryKey: ['notebooks'], queryFn: () => api<Notebook[]>('/notebooks') })
  const create = useMutation({ mutationFn: () => api<Notebook>('/notebooks', { method: 'POST', body: JSON.stringify({ title }) }), onSuccess: () => { setTitle(''); void client.invalidateQueries({ queryKey: ['notebooks'] }) } })
