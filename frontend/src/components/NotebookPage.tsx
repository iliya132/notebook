...
export function NotebookPage() {
  ...
  return <main className="page"><Link className="back" to="/app">← Все книжки</Link><div className="page-heading"><div><p className="eyebrow">Записная книжка</p><h1>{query.data.title}</h1></div><div className="toolbar-actions"><button className="ghost" onClick={() => { setBookTitle(query.data.title); setRenaming(value => !value) }}>Переименовать</button><button className="danger ghost" onClick={() => { if (window.confirm('Удалить книжку и все заметки? Это действие нельзя отменить.')) remove.mutate() }}>Удалить книжку</button></div></div>
    ...
    <div className="note-list">
      <button className="create-note" type="button" disabled={create.isPending} onClick={() => create.mutate()} aria-label="Создать заметку"><span className="create-icon" aria-hidden="true">+</span><span><strong>{create.isPending ? 'Создаём заметку…' : 'Новая заметка'}</strong><small>Откроется сразу в редакторе</small></span></button>
      {query.data.notes.map(note => <Link to={`/app/notes/${note.id}`} key={note.id} onMouseEnter={() => void prefetchNote(note.id)} onFocus={() => void prefetchNote(note.id)}><div><h2>{note.title}</h2><time>{formatDateTime(note.updatedAt)}</time></div><span>→</span></Link>)}
    </div>
  </main>
}
